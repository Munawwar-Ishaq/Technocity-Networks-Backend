...
const Controller = async (req, res) => {
  ...
  try {
    let respon = await addUser(req.body);
    ...
    let totalAmount = parseFloat(req.body.packageRate || 0);
    if (req.body.staticIP) {
      totalAmount += parseFloat(req.body.staticIPAmmount || 0);
    }

    let amountPaid = parseFloat(req.body.amountPaid || 0);
